...
function scrollbarText(scrollTop: number, scrollHeight: number, clientHeight: number, railHeight: number): string {
  ...
  const thumbTop = Math.min(height - thumbHeight, Math.floor((scrollTop / max) * (height - thumbHeight)));
  let text = '';
...
function makeLoremDocument(): string {
  ...
  for (let index = 0; index < 36; index += 1) {
    sections.push(`${String(index + 1).padStart(2, '0')}. ${paragraphs[index % paragraphs.length]}`);
  }
